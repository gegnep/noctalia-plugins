...
| 0007 button radius/padding | Exposes `radius`/`padding`/`paddingH`/`paddingV` on declarative `button` nodes (Button already inherits Flex's setters; only the allowlist was missing). Lets niri-taskbar render capsule workspace chips with a real active-pill inset. Upstream: [noctalia-dev/noctalia#3355](https://github.com/noctalia-dev/noctalia/pull/3355) (branch `plugin-button-shape`). |
